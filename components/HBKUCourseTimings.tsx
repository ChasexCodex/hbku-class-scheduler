...
const HBKUCourseTimings = ({crns, courses, timings, handleTimingsChange, handleAddTiming, handleRemoveTiming}: Props) => {
  return (
    <div className="rounded-lg border-2 border-gray-400 bg-zinc-800 px-4 pb-4 pt-2">
      <p className="text-xl mb-4 font-semibold text-gray-100">HBKU Courses Timings</p>
      <div className="space-y-2">
        {crns?.map(e => getHBKUCourseDetails(e, courses)!)
            .filter(e => e)
            .map(e => (
              <div key={e.crn} className="space-y-2 border-2 p-2 w-full bg-zinc-700 rounded-md border-gray-600">
                <p className="text-gray-100">{e.name} ({e.title})</p>
                {timings[e.crn]?.map((t, i) => (
                    <div key={i}
                         className="relative flex flex-row justify-between text-sm bg-zinc-500 p-2 rounded-md border">
                      <button
                        type="button"
                        onClick={() => handleRemoveTiming(e.crn, i)}
                        className="absolute top-1 right-1 bg-red-500 rounded-full h-5 w-5 p-0.5 leading-3 font-bold">
                        x
                      </button>
                      <label className="flex flex-col gap-2">
                        ...
                        <input
                          ...
                          className="bg-zinc-900 text-gray-300 px-2 py-1 rounded-md border border-gray-600"
                      ...
                      <label className="flex flex-col gap-2">
                        ...
                        <select
                          defaultValue={t.day}
                          className="bg-zinc-900 text-gray-300 h-min py-1 px-2 rounded-md border border-gray-600"
                  ...
                  <p className="text-gray-300">No timings set</p>
                }
                <button
                  type="button"
          ...
          <p className="text-gray-300">No HBKU courses listed</p>
        }
      </div>
    </div>
  )
